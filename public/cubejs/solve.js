(function () {
  ...
  factorial = function (n) {
    var f, i, m, ref
    f = 1
    for (i = m = 2, ref = n; ref >= 2 ? m <= ref : m >= ref; i = ref >= 2 ? ++m : --m) {
      f *= i
    }
    return f
  }
  ...
  permutationIndex = function (context, start, end, fromEnd = false) {
    ...
    our = (function () {
      var m, ref, results
      results = []
      for (i = m = 0, ref = maxOur; ref >= 0 ? m <= ref : m >= ref; i = ref >= 0 ? ++m : --m) {
        results.push(0)
      }
      return results
    })()
    return function (index) {
      ...
      if (index != null) {
        ...
        perm = this[permName]
        for (i = o = 0, ref1 = maxAll; ref1 >= 0 ? o <= ref1 : o >= ref1; i = ref1 >= 0 ? ++o : --o) {
          perm[i] = -1
        }
        // Generate permutation from index b
        for (j = p = 1, ref2 = maxOur; ref2 >= 1 ? p <= ref2 : p >= ref2; j = ref2 >= 1 ? ++p : --p) {
          k = b % (j + 1)
          b = (b / (j + 1)) | 0
        ...
        x = maxOur
        if (fromEnd) {
          for (j = q = 0, ref3 = maxAll; ref3 >= 0 ? q <= ref3 : q >= ref3; j = ref3 >= 0 ? ++q : --q) {
            c = Cnk(maxAll - j, x + 1)
            if (a - c >= 0) {
              perm[j] = our[maxOur - x]
              a -= c
        ...
        perm = this[permName]
        for (i = u = 0, ref5 = maxOur; ref5 >= 0 ? u <= ref5 : u >= ref5; i = ref5 >= 0 ? ++u : --u) {
          our[i] = -1
        }
        a = b = x = 0
        ...
        if (fromEnd) {
          ...
          for (j = y = 0, ref8 = maxAll; ref8 >= 0 ? y <= ref8 : y >= ref8; j = ref8 >= 0 ? ++y : --y) {
  ...
  computeMoveTable = function (context, coord, size) {
    ...
    cube = new Cube()
    results = []
    for (i = m = 0, ref = size - 1; ref >= 0 ? m <= ref : m >= ref; i = ref >= 0 ? ++m : --m) {
      cube[coord](i)
      inner = []
      for (j = o = 0; o <= 5; j = ++o) {
        move = Cube.moves[j]
  ...
  computePruningTable = function (phase, size, currentCoords, nextIndex) {
    ...
    table = (function () {
      var m, ref, results
      results = []
      for (x = m = 0, ref = Math.ceil(size / 8) - 1; ref >= 0 ? m <= ref : m >= ref; x = ref >= 0 ? ++m : --m) {
    ...
    // depth.
    while (done !== size) {
      for (index = m = 0, ref = size - 1; ref >= 0 ? m <= ref : m >= ref; index = ref >= 0 ? ++m : --m) {
  ...
  Cube.prototype.solveUpright = function (maxDepth = 22) {
    ...
    phase1search = function (state) {
      ...
      depth = 0
      results = []
      for (depth = m = 1, ref = maxDepth; ref >= 1 ? m <= ref : m >= ref; depth = ref >= 1 ? ++m : --m) {
        phase1(state, depth)
        if (solution !== null) {
          break
        }
    ...
    phase2search = function (state) {
      ...
      state.init2()
      results = []
      for (depth = m = 1, ref = maxDepth - state.depth; ref >= 1 ? m <= ref : m >= ref; depth = ref >= 1 ? ++m : --m) {
        phase2(state, depth)
        if (solution !== null) {
          break
        }
    ...
    freeStates = (function () {
      var m, ref, results
      results = []
      for (x = m = 0, ref = maxDepth + 1; ref >= 0 ? m <= ref : m >= ref; x = ref >= 0 ? ++m : --m) {
